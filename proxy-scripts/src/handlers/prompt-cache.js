...
export function prepareToolsForPromptCache(tools, providerKind, options = {}) {
  ...
  const provider = String(providerKind || "").trim().toLowerCase();
  const config = options.config || getPromptCacheConfig();
...
function applyMessagesPrefixCache(messages, tailMessages, additionalTailMessages = 0) {
  ...
  const volatileTailMessages = Number.isInteger(additionalTailMessages) ? Math.max(0, additionalTailMessages) : 0;
...
export function applyAnthropicPromptCache(body, options = {}) {
  ...
  if (Array.isArray(next.messages)) {
    next.messages = applyMessagesPrefixCache(next.messages, config.tailMessages, Number.parseInt(String(config.additionalTailMessages || "0"), 10));
  }
  return next;
}
...
export function shouldRetryWithoutPromptCache(statusCode, bodyText) {
  ...
  return /cache_control|prompt caching|prompt_cache|cached content|cache control|cache breakpoint|cache\.control|ephemeral cache/.test(text);
}
