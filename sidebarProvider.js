...
class SidebarProvider {
  ...
  async fetchModelsFromGateway(tmp02, tmp1) {
    ...
    if (tmp2) {
      return this.httpGetModels(this.getModelListUrl(tmp2), tmp02, 8000);
    }
  ...
  async probeConfiguredModelStream(tmp02) {
    ...
    const tmp6 = /^https?:\/\//i.test(tmp5) ? tmp5.replace(/\/+$/, "") : "https://" + stripProtoServer(tmp5);
  ...
  async checkManagedEnvironment() {
    ...
    const tmp24 = !!tmp22.path && tmp23.some(arg0 => arg0.status === "available");
    tmp02.push(this.envCheckItem("devin-patch", "Devin Desktop 补丁", tmp23.length === 0 ? "ok" : "warning", !tmp22.path ? "未找到 Devin Desktop extension.js" : tmp23.length === 0 ? "已安装" : "未就绪 " + tmp23.length + "/" + tmp22.patches.length, tmp24));
    return {
  ...
  async repairManagedEnvironment() {
    ...
    if (tmp5.path && tmp5.patches.some(arg0 => arg0.status === "available")) {
...
function stripProtoServer(arg0) {
  return arg0.replace(/^https?:\/\//, "").replace(/\/+$/, "");
}
