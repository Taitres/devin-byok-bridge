...
class SidebarProvider {
  ...
  async applySavedConfig(tmp02, {
    ...
    if (tmp4.running && this.validateByokSlots(tmp02).length === 0) {
      ...
      if (tmp22) {
        ...
        tmp7 = true;
        if (tmp32) {
          const tmp42 = "端口已变更；如需让 Devin Desktop 使用新端口，请手动点击“安装补丁”并重载窗口";
          if (silent) {
            this.logLines.push(tmp42);
          } else {
            this.postActionState("patch", "success", tmp42);
          }
        }
      } else {
        const tmp05 = {
    ...
    if (!silent) {
      this.postActionState("config", tmp6 ? "error" : "success", tmp5);
    } else if (tmp6) {
      this.logLines.push(tmp5);
      if (this.logLines.length > 200) {
        this.logLines = this.logLines.slice(-100);
      }
      this.view?.webview.postMessage({
  ...
  async handleMessage(tmp02) {
    switch (tmp02.command) {
      ...
      case "saveConfig":
        {
          await this.applySavedConfig(tmp02.config, {
            silent: tmp02.silent === true
          });
          break;
        }
